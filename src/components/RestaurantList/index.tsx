...
const RestaurantList: FC = (): JSX.Element => {
  ...
  const { data, isLoading, error } = useSelector<IRestaurantsInitialState>(
        ...
        {isLoading ? (
          ...
          data.map((restaurant, index) => (
            <div
              ...
              key={index}
              onClick={() =>
                getLocationLatLng(restaurant.address, restaurant._id)
              }
            >
              <div style={{ padding: '10px 10px 8px 10px' }}>
                <div className={s.title}>{restaurant.name}</div>
                <Rating data={restaurant.rating} size={16} disabled />
              </div>
              <Button
